...
export const fetchUsers = createAsyncThunk<User[]>(
  ...
  async (_, thunkAPI) => {
    try {
      const res = await fetch("http://192.168.254.106:2701/users/get-all-users", {
        headers: {
...
export const fetchUserById = createAsyncThunk<User, string>(
  ...
  async (id, thunkAPI) => {
    try {
      const res = await fetch(`http://192.168.254.106:2701/users/get-user/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  async (user, thunkAPI) => {
    try {
      const res = await fetch(
        `http://192.168.254.106:2701/users/update-user/${user.id}`,
        {
          method: "PUT",
          headers: {
...
export const deleteUserAsync = createAsyncThunk<string, string>(
  ...
  async (id, thunkAPI) => {
    try {
      const res = await fetch(`http://192.168.254.106:2701/users/delete-user/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
